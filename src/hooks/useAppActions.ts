...
export const useAppActions = ({
    ...
    const handleVoteWithRange = useCallback(async (proposalId: number, lower: number, upper: number, power: number) => {
        ...
        try {
            ...
            if (success) {
                 console.log(`Simulated vote on ${proposalId} by ${session!.user.address} with range [${lower}, ${upper}] power ${power}`);
    ...
    }, [checkConnection, setLoading, showSnackbar, setUserBalances]);
    ...
    const handleRemoveLiquidity = useCallback(async (tokenA: string, tokenB: string, lpAmount: number) => {
        ...
        setLoading('removeLiquidity', false);
    }, [checkConnection, userBalances, setLoading, showSnackbar, setUserBalances]);
