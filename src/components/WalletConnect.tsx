...
import { Paper, Typography, TextField, Button, Box, CircularProgress, Alert, IconButton, Tooltip, Divider, SvgIcon } from '@mui/material';
...
const WalletConnect: React.FC = () => {
    ...
    const { authentication, isConnecting, authError } = useAuthContext();

    // --- Local State ---
    const [mnemonic, setMnemonic] = useState("");
    const [localError, setLocalError] = useState<string | null>(null); // For form validation
    const [copied, setCopied] = useState(false);
    const [isMetaMaskAvailable, setIsMetaMaskAvailable] = useState(false);
    const [isSimulatingLoading, setIsSimulatingLoading] = useState(false); // Button specific loading
    const [isMetaMaskLoading, setIsMetaMaskLoading] = useState(false); // Button specific loading

    useEffect(() => { if (typeof window !== 'undefined' && window.ethereum) setIsMetaMaskAvailable(true); }, []);
    ...
    const handleConnectSimulation = () => {
        clearLocalError();
        if (!validateMnemonic(mnemonic)) { setLocalError("Invalid input. Please enter exactly 16 words."); return; }
        if (isSimulatingLoading || isMetaMaskLoading || isConnecting) return;
        setIsSimulatingLoading(true);
        // Calculate simulated address
        let hash = 0;
        const phraseToHash = mnemonic.trim().toLowerCase();
        for (let i = 0; i < phraseToHash.length; i++) { hash = ((hash << 5) - hash) + phraseToHash.charCodeAt(i); hash |= 0; }
        const simulatedAddress = "0x" + Math.abs(hash).toString(16).padStart(40, 'a').substring(0, 40);
        // Call context signIn - it handles the global isConnecting state
        authentication.signIn(simulatedAddress, null, 'simulated');
        // Reset button loading state after a short delay to match signIn's internal timeout
        setTimeout(() => setIsSimulatingLoading(false), 600);
    };

    const handleConnectMetaMask = async () => {
        clearLocalError();
        if (!isMetaMaskAvailable || !window.ethereum) { setLocalError("MetaMask is not available."); return; }
        if (isSimulatingLoading || isMetaMaskLoading || isConnecting) return;
        ...
        try {
            ...
            if (accounts && accounts.length > 0) {
                // Call context signIn
                authentication.signIn(accounts[0], accounts, 'metamask');
            } else { setLocalError("No accounts found in MetaMask."); }
        } catch (err: any) {
            console.error("MetaMask connection error:", err);
            if (err.code === 4001) setLocalError("Connection request rejected.");
            else setLocalError(err.message || "Failed to connect with MetaMask.");
        } finally {
            // Reset button loading state after a short delay
             setTimeout(() => setIsMetaMaskLoading(false), 600);
        }
    };

    const handleUseExample = () => { setMnemonic(EXAMPLE_MNEMONIC); clearLocalError(); };
    const handleCopyExample = () => {
        navigator.clipboard.writeText(EXAMPLE_MNEMONIC).then(() => { setCopied(true); setTimeout(() => setCopied(false), 2000); })
            .catch(err => { console.error('Copy failed: ', err); setLocalError("Failed to copy."); });
    };

    // Use global connecting state + button specific loading
    const anyConnectionLoading = isConnecting || isSimulatingLoading || isMetaMaskLoading;
    const displayError = localError || authError; // Show local validation or auth context error
    ...
    return (
        <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '100vh', p: 2, bgcolor: 'background.default' }}>
            <Paper elevation={3} sx={{ padding: { xs: 2, sm: 4 }, maxWidth: 480, width: '100%', borderRadius: 2 }}>
                ...
                {displayError && <Alert severity="error" sx={{ mb: 2 }} onClose={() => { clearLocalError(); /* Can't clear authError here easily */ }}>{displayError}</Alert>}

                {/* MetaMask Option */}
                <Box sx={{ mb: 3 }}>
                    <Typography variant="subtitle1" align="center" sx={{ mb: 1.5 }}>Connect with Browser Wallet</Typography>
                    <Button variant="contained" color="primary" fullWidth onClick={handleConnectMetaMask} disabled={!isMetaMaskAvailable || anyConnectionLoading} sx={{ py: 1.5, fontSize: '1rem' }} startIcon={isMetaMaskLoading ? <CircularProgress size={20} color="inherit" /> : <MetaMaskIcon />}>
                        {isMetaMaskLoading ? 'Connecting...' : (isMetaMaskAvailable ? 'Connect MetaMask' : 'MetaMask Not Found')}
                    </Button>
                    {!isMetaMaskAvailable && (<Typography variant="caption" display="block" color="text.secondary" sx={{mt: 1, textAlign: 'center'}}>Please install MetaMask.</Typography>)}
                </Box>

                <Divider sx={{ my: 3 }}><Typography variant="overline">OR</Typography></Divider>

                {/* Simulation Option */}
                <Box>
                    <Typography variant="subtitle1" align="center" sx={{ mb: 1 }}>Use Simulation (Testing Only)</Typography>
                    <Alert severity="warning" icon={<InfoOutlinedIcon fontSize="inherit" />} sx={{ mb: 2 }}>For testing purposes. <strong>Do NOT enter real recovery phrases.</strong></Alert>
                    <TextField id="mnemonic-input" label="Secret Recovery Phrase (16 words)" placeholder="Enter the 16-word phrase..." multiline minRows={3} fullWidth value={mnemonic} onChange={(e) => setMnemonic(e.target.value)} margin="normal" disabled={anyConnectionLoading} />
                    <Button variant="outlined" color="secondary" fullWidth onClick={handleConnectSimulation} disabled={anyConnectionLoading || !validateMnemonic(mnemonic)} sx={{ mt: 1, mb: 2, py: 1.5 }} startIcon={isSimulatingLoading ? <CircularProgress size={20} color="inherit" /> : null}>
                        {isSimulatingLoading ? 'Connecting...' : 'Connect Simulated Wallet'}
                    </Button>
                    <Paper variant="outlined" sx={{ p: 1.5, mb: 1, backgroundColor: "action.hover", position: 'relative', wordBreak: 'break-word', mt: 1 }}>
                        <Typography variant="body2" component="code" sx={{ fontFamily: 'monospace' }}>{EXAMPLE_MNEMONIC}</Typography>
                        <Tooltip title={copied ? "Copied!" : "Copy example phrase"} placement="top">
                            <IconButton size="small" onClick={handleCopyExample} sx={{ position: 'absolute', top: 4, right: 4 }} aria-label="copy example mnemonic"><ContentCopyIcon fontSize="inherit" /></IconButton>
                        </Tooltip>
                    </Paper>
                    <Button size="small" variant="text" fullWidth onClick={handleUseExample} disabled={anyConnectionLoading}>Use Example Phrase Above</Button>
                </Box>
            </Paper>
        </Box>
    );
